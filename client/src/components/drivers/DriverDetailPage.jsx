...
const DriverDetailPage = () => {
  ...
  const canViewUserList = authUser?.role === 'admin' || authUser?.role === 'owner';
  ...
  const fetchDriverData = useCallback(async () => {
    ...
    try {
      ...
      if (ordersRes.status === 'fulfilled') {
        setTrips(ordersRes.value.data?.data || ordersRes.value.data?.orders || []);
      }
      ...
      setError(err.response?.data?.error || err.response?.data?.message || err.message || 'Failed to load driver.');
    } finally {
      setLoading(false);
    }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.response?.data?.message || 'Creation failed');
    } finally {
      setSubmitting(false);
    }
  };
              ...
              <form onSubmit={isCreateMode ? handleCreate : handleUpdate}>
                {isCreateMode && (
                  <div className="form-group mb-2">
                    ...
                    {!canViewUserList && (
                      <small className="text-danger">Only admins/owners can view user accounts for driver creation.</small>
                    )}
                    {canViewUserList && driverUsers.length === 0 && !error && (
                      <small className="text-danger">No eligible user accounts found.</small>
                    )}
                  </div>
                )}
            ...
            <div className="card-body">
              {driver?.assignedVehicle ? (
                <div>
                  ...
                  {location && <p className="text-sm text-muted">Live location available</p>}
                </div>
              ) : (
              ...
              <tbody>
                {trips.map((t) => (
                  <tr key={t._id}>
                    ...
                    <td>
                      <span className={`badge ${orderStatusBadge(t.status)}`}>{t.status}</span>
                    </td>
                ...
                {trips.length === 0 && (
                  <tr>
                    <td colSpan="3" className="text-center text-muted">No trips found</td>
                  </tr>
                )}
              </tbody>
            </table>
